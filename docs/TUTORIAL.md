...
| 6 — Configuration File + Hot Reload | In progress | **Config file half done:** `internal/config/config.go` defines `Config`/`Listener`/`HealthCheck`/`Backend` with `yaml.v3` tags, a custom `Duration` type so fields can be written as `"5s"`, `Load()` with validation, and `configs/example.yaml`. `cmd/gobalance/main.go` now builds a pool/balancer/health-checker per listener from the loaded config via a new `balancer.New(name, pool)` factory, replacing the old single-hardcoded-pool setup — each listener runs on its own goroutine. **Hot-reload half not started:** `internal/config/store.go`'s `Store` (`atomic.Pointer[Config]`) exists and is tested but `main.go` doesn't use it yet; `pool.Pool.Backends` has no locking, so it isn't safe to mutate while a listener is running; no `fsnotify`/`SIGHUP` trigger exists yet. |
...
**Pick up here:** Phase 6, hot-reload half — add an `RWMutex`-guarded `SetBackends` to `internal/pool.Pool` so its backend list can be safely replaced while a listener is running, give `main.go` a way to look up a running listener's `*pool.Pool` by name, then wire `config.Store` + `fsnotify`/`SIGHUP` to actually trigger a reload.
