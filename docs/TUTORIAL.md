...
| 10 — Containerize and Package for Demo | Done | `deploy/Dockerfile` is a two-stage build (`golang:1.26` compiles a static `CGO_ENABLED=0` binary, `gcr.io/distroless/static-debian12` runs it) so the shipped image carries no shell or package manager. `configs/docker.yaml` is a copy of `example.yaml` with backend addresses swapped from `localhost:910x` to the compose service names (`backend1:5678` etc.) — containers don't share gobalance's localhost namespace the way the manual-testing dummy backends do, so the two configs can't be the same file. `deploy/docker-compose.yaml` wires gobalance up with three `hashicorp/http-echo` containers (replacing the ad-hoc `python3 -m http.server` dummy backends) and maps `8080→9091` (L7), `9090→9090` (L4), `9100→9100` (metrics). The demo runs with `-tls=false` for a plain `curl` checkpoint, matching the tutorial's own script. `docker build --target build` verified the compile stage succeeds; the final `distroless` stage couldn't be pulled from this sandbox's network (`gcr.io` unreachable here) but the Dockerfile shape is the same one from this file, so `docker compose -f deploy/docker-compose.yaml up` from a normal machine is the real checkpoint to run. |
| 11 — Polish for Portfolio | Not started | |

**Pick up here:** Phase 11 — polish for portfolio. Write a top-level README pitch (partially done — see README's Status section), pull in the architecture diagram from TECH_STACK.md, record a short failover-demo clip, and tag `v1.0` once M1–M8 in the PRD are all checked off.
